...
namespace VRCEyeTracking.QuickMenu
{
    public static class QuickModeMenu
    {
        ...
        private static QuickMenuTab EyeTab, MouthTab;

        public static void InitializeMenu()
        {
             CreateNotificationTab("VRCSRanipal", "Text", Color.green,
        ...
        {
            var originalTabs = GameObject.Find(
                "UserInterface/QuickMenu/QuickModeMenus/QuickModeNotificationsMenu/NotificationTabs");
            
            var newTabs =
                GameObject.Instantiate(originalTabs);
            
            newTabs.transform.parent =
                GameObject.Find("UserInterface/QuickMenu/QuickModeMenus/QuickModeNotificationsMenu").transform;

            newTabs.transform.localScale = originalTabs.transform.localScale;
            newTabs.transform.localPosition = originalTabs.transform.localPosition;
            newTabs.transform.localRotation = originalTabs.transform.localRotation;
            
            newTabs.GetComponent<MonoBehaviourPublicObCoGaCoGaObCoObCoObUnique>().enabled = true;
            originalTabs.transform.parent = newMenu;

            var newOriginalTabs = GameObject.Find("UserInterface/QuickMenu/QuickModeMenus/VRCSRanipalMenu/NotificationTabs");
            
            //Strip away notif menu scripts and fix button colors
            for (var i=0;
            i < newOriginalTabs.transform.GetChildCount();i++)
            {
                var tab = newOriginalTabs.transform.GetChild(i).gameObject;

                switch (tab.gameObject.name)
                {
                    case "InvitesTab":
                        EyeTab = new QuickMenuTab(tab.gameObject, "Eye Tracking");
                        EyeTab.Bind(() =>
                        {
                            for (var j=0; j<SRanipalTrack.LatestEyeData.verbose_data.tracking_improvements.count; j++)
                                MelonLogger.Msg(SRanipalTrack.LatestEyeData.verbose_data.tracking_improvements.items[j]);
                        });
                        break;
                    case "FriendRequestsTab":
                        MouthTab = new QuickMenuTab(tab.gameObject, "Mouth Tracking");
                        break;
                    default:

...
namespace VRCFaceTracking
{
    public class ConfigParser
    {
        ...
        public async void ParseCurrentAvatar(IPEndPoint oscQueryEndpoint)
        {
            ...
            var targetParams = avatarConfig.Contents["parameters"].Contents.Where(p => (int)p.Value.Access > 1)
                .Select(entry => (entry.Key, entry.Value.FullPath, 
                    OscUtils.TypeConversions.First(t => 
                        t.Key.typeChar
                            .Contains(entry.Value.OscType
                                .First()))
                        .Key
                        .Item1))
                .ToArray();
